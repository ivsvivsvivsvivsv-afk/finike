"""
🎮 Telegram Bot для Игрового Дня — v2.2 WEBHOOK версия (ИСПРАВЛЕННАЯ)
Регистрация на игры: Катан, Каркассон, D&D

WEBHOOK MODE - РАБОТАЕТ НА AMVERA С ПРАВИЛЬНЫМ HTTPS URL
"""
...
# ВАЖНО! Для Amvera нужен правильный HTTPS URL
# Формат: https://finike-games-bot-<random>.amvera.io/webhook
AMVERA_APP_ID = os.getenv("AMVERA_APP_ID", "finike-games-bot")  # Замени на свой app id
WEBHOOK_URL = f"https://{AMVERA_APP_ID}.amvera.io{WEBHOOK_PATH}"
...
async def cb_time_selected(query: types.CallbackQuery):
    ...
    logger.info(f"✅ Регистрация: {user.username} на {game_info['name']} в {time_slot}")
...
async def handle_webhook(request: web.Request):
    ...
    try:
        ...
        logger.info(f"📨 Обновление получено и обработано")
    except Exception as e:
...
async def on_startup(app):
    ...
    # ВАЖНО! Устанавливаем вебхук с правильным HTTPS URL
    try:
        await bot.set_webhook(url=WEBHOOK_URL)
        logger.info(f"✅ Вебхук установлен: {WEBHOOK_URL}")
    except Exception as e:
        logger.error(f"❌ ОШИБКА установки вебхука: {e}")
        logger.info(f"💡 Проверь что WEBHOOK_URL правильный: {WEBHOOK_URL}")
        logger.info(f"💡 Должен быть формат: https://app-name.amvera.io/webhook")
